    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto space-y-8">
      ...
      >
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          {/* Score Badge */}
          <div className="flex flex-col items-center bg-white/10 backdrop-blur-md border border-white/20 rounded-3xl p-6 sm:p-8 min-w-[220px] text-center shadow-lg">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          ...
          <span className="text-[11px] text-slate-500">Pass mark: {exam?.passingMarks}</span>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          ...
          <span className="text-[11px] text-slate-500">+{exam?.negativeMarking ? 'Full marks' : 'Points earned'}</span>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          ...
          <span className="text-[11px] text-slate-500">Allowed: {exam?.duration} mins</span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-sm rounded-xl transition-colors shadow-xs"
          >
        ...
        <div className="flex items-center gap-3">
          <Link
            to="/results"
            className="px-5 py-2.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold text-sm rounded-xl transition-colors shadow-xs"
          >
            All Results History
          </Link>
          <Link
            to="/dashboard"
            className="inline-flex items-center gap-1.5 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm rounded-xl transition-colors shadow-md shadow-indigo-200"
          >
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between pb-2 border-b border-slate-200">
            ...
            <span className="text-xs font-semibold text-slate-500">
              Review answers, correct solutions, and explanations
            </span>
          </div>
          ...
          {answers.map((ans, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`rounded-2xl border ${borderClass} p-6 shadow-xs transition-all space-y-4`}
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {ans.options?.map((opt, optIdx) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span className="w-5 h-5 rounded-full bg-white/80 border flex items-center justify-center font-bold text-[10px]">
